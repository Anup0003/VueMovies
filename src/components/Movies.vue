...
export default {
  ...
  methods: {
    async fetchTopMovies() {
      try {
        const response = await tmdb.getTopMovies();
...
<template>
      ...
      <div class="details ml-14">
        ...
        <v-card-text>
          <div class="d-flex mb-4 mt-4">
            <v-icon color="yellow" class="pb-1 mr-1 star">mdi-star</v-icon>
            <div class="vote">
              <p> {{ getAverageVote(currentMovie) }}</p>
            </div>
          ...
          <p class="overview font-weight-medium"> {{ getOverview(currentMovie.overview) }}</p>
        </v-card-text>
        <div class="actors">
          <v-card-title>Actors</v-card-title>
          <ul class="d-flex ac-list ml-3">
            <li v-for="actor in currentMovie.actors" :key="actor.id" class="pr-3">
  ...
  <div class="carousel">
    <v-sheet class="mx-auto sheet" elevation="0" max-width="710" color="transparent">
      <h3 class="text-light-blue-lighten-5 ml-16 mt-6">Top Movies</h3>
      <v-slide-group v-model="model" class="pa-4" center-active show-arrows>
        <v-slide-group-item v-for="movie in remainingMovies" :key="movie.id">
          <v-card color="grey-lighten-1" class="ma-4" height="100" width="140" @click="selectMovie(movie)">
